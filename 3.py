import os
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
from sklearn.decomposition import PCA
from sklearn.preprocessing import StandardScaler

# =========================================================================
# 1. 自定义设置你的 Excel 文件存放位置 (支持绝对路径)
# =========================================================================
DATA_FOLDER = r"C:/Users/Surface pro8/Desktop/多晶"

file_list = [
    "jh.xlsx",
    "wzh.xlsx",
    "ljl.xlsx",
    "syf.xlsx",
    "xdh.xlsx",
    "ypg.xlsx",
    "zbs.xlsx",
]

dfs = {}
for f in file_list:
    full_path = os.path.join(DATA_FOLDER, f)
    if not os.path.exists(full_path):
        csv_backup_name = f.replace(".xlsx", "") + " - Sheet1.csv"
        csv_full_path = os.path.join(DATA_FOLDER, csv_backup_name)
        if os.path.exists(csv_full_path):
            df_temp = pd.read_csv(
                csv_full_path, header=None, skiprows=1, names=["x", "y"]
            )
        else:
            raise FileNotFoundError(f"找不到文件: {f}")
    else:
        df_temp = pd.read_excel(full_path)
        df_temp.columns = ["x", "y"]
    dfs[f] = df_temp

# =========================================================================
# 2. 数据对齐与归一化
# =========================================================================
base_x = dfs[file_list[0]]["x"].values
combined_df = pd.DataFrame({"x": base_x})
for f in file_list:
    name = f.split(".")[0]
    combined_df[name] = dfs[f]["y"]

cleaned_df = combined_df.dropna().reset_index(drop=True)
x_clean = cleaned_df["x"].values
Y_signals = cleaned_df.drop(columns=["x"]).values

scaler = StandardScaler()
Y_scaled = scaler.fit_transform(Y_signals)

# =========================================================================
# 3. PCA 提取第一主成分波形 (PCA1)
# =========================================================================
pca = PCA(n_components=1)
Y_pca = pca.fit_transform(Y_scaled)
Y_reconstructed = pca.inverse_transform(Y_pca)
y_pca1 = np.mean(Y_reconstructed, axis=1)

# =========================================================================
# 4. 核心修改：直接指定图片中的 10 个峰对应的横坐标数据
# =========================================================================
image_peak_x = [6.2495, 7.207, 10.24535, 12.03453, 12.5828,14.56819,15.907, 16.335, 17.939, 19.07012,20.84,21.84,22.16,23.437,24.35,24.65,25.826]

# =========================================================================
# 5. 可视化绘图展示（单大图、多色竖虚线、无小框注释）
# =========================================================================
plt.figure(figsize=(12, 6), dpi=120)
plt.rcParams["font.sans-serif"] = ["SimHei"]
plt.rcParams["axes.unicode_minus"] = False

# 绘制 PCA1 主线
plt.plot(x_clean, y_pca1, color="black", linewidth=2.5, label="PCA1 信号波形")

# -------------------------------------------------------------------------
# 核心视觉映射：按人类习惯的第 1~10 根虚线分别配置颜色与粗细
# -------------------------------------------------------------------------
for idx, px in enumerate(image_peak_x):
    peak_number = idx + 1  # 对应第 1~10 根线

    if peak_number in [2, 3, 8]:
        # 层级一：最鲜艳突出的深红色
        line_color = "crimson"
        line_width = 1.2
    elif peak_number in [1, 5, 6, 9]:
        # 层级二：次显著的皇家蓝
        line_color = "royalblue"
        line_width = 1.0
    else:
        # 层级三：视觉最弱的淡灰色
        line_color = "lightgray"
        line_width = 0.8

    # 在对应横坐标位置绘制精细竖虚线
    plt.axvline(
        x=px, color=line_color, linewidth=line_width, linestyle="--", alpha=0.9
    )

    # 在图表顶部边缘标记对应的编号（如 #1, #2...），方便对齐肉眼核对
    plt.text(
        px,
        max(y_pca1) * 1.01,
        f"#{peak_number}",
        color=line_color,
        fontsize=8,
        ha="center",
        alpha=0.75,
    )

# 基础图表清洗，去掉所有选区框和右边的小图
plt.title("PCA1 信号多层级核心峰位置对应图", fontsize=12, pad=15)
plt.xlabel("角度", fontsize=10)
plt.ylabel("标准幅值", fontsize=10)
plt.grid(False)  # 移除全局背景网格，使虚线不受干扰
plt.tight_layout()
plt.show()